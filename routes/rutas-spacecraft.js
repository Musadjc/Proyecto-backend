...
router.post("/", async (req, res, next) => {
    ...
    const { yearProduccion, launch, nombre, altura, anchura, masa, empuje } = req.body;
    const nuevoSpacecraft = new Spacecraft({
    ...
    if (!launchBusca) {
      const error = new Error(
        "No existe un launch con este id"
      );
      error.code = 404;
      return next(error);
    }
    ...
    try {
      ...
      launchBusca.spacecraft.push(nuevoSpacecraft); // ? (2)
